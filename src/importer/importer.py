...
class AIFDataset(InMemoryDataset):
    ...
    def get(self, idx):
        data = torch.load(
            os.path.join(self.processed_dir, f"{self.processed_file_names[idx]}")
        )
        return data
    ...
    def _process_file(self, file_path):
        ...
        with file_lock:
            if processed_file_name in os.listdir(self.processed_dir):
                thread_utils.thread_safe_print(
                    f"Processed file {processed_file_name} already exists. Skipping..."
                )
        ...
        try:
            ...
            for node in json_data["nodes"]:
                graph.add_node(
                    node["nodeID"], type=node["type"], text=node["text"], embedding=None
                )
        ...
        if self.pre_transform is not None:
            try:
                ...
                thread_utils.thread_safe_print(
                    f"Failed to apply pre-transform for sub-graph {aif_data.name}: {str(transform_error)}"
                )
                return
        ...
        for i, component in enumerate(connected_components):
            ...
            component_data = AIFData(
                graph=subgraph, name=subgraph_name, num_nodes=subgraph.number_of_nodes()
            )

            # Filter components
            if self.pre_filter is not None and not self.pre_filter(component_data):
                thread_utils.thread_safe_print(
                    f"Pre-filter rejected data in sub-graph {component_data.name}. Skipping this file."
                )
                continue
        ...
        for component_data in subgraph_data:
            with file_lock:
                save_path = os.path.join(
                    self.processed_dir, f"data_{component_data.name}.pt"
                )
    ...
    def graph_to_pyg(self, data):
        try:
            ...
            node_embeddings_tensor = node_embeddings_tensor.view(
            ...
            sentiment_values = [
                nodes[node].get("sentiment", 0.0) for node in data.graph.nodes
            ]
    ...
    def edge_label_encoder(self, data):
        ...
        try:
            unique_labels_before = set(
                [graph.edges[src, dst].get("type") for src, dst in graph.edges()]
            )
            thread_utils.thread_safe_print(
                f"Unique edge labels before encoding: {unique_labels_before}"
            )
            ...
            label_index = [
                self.label_to_index[graph.edges[src, dst].get("type")]
            ...
            thread_utils.thread_safe_print(
                f"Failed to encode edge labels for {data.name}: {str(e)}"
            )
